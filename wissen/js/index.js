	...
	sketch.setup = () => {
		...
		for (let x = 0; x < ig.width; x++) {
			for (let y = 0; y < ig.height; y++) {
				...
				let wavR = sketch.map(sketch.sin(sketch.radians(22 + x * 4 + y)), -1, 1, 0, 0);
				let wavG = sketch.map(sketch.sin(sketch.radians(22 + x * 4 + y)), -1, 1, 255, 0);
				let wavB = sketch.map(sketch.sin(sketch.radians(22 + x * 4 + y)), -1, 1, 0, 255);
	...
	sketch.setup = () => {
		...
		for (let x = 0; x < ig.width; x++) {
			for (let y = 0; y < ig.height; y++) {
				...
				// you can put the ig.pixels into variable and get crazy :)
				// Since we are creating the pixels, don't need to "access" it beforehand.
				let wavR = sketch.map(sketch.sin(sketch.radians(250 + x * 0.8 + y)), -1, 1, 0, 0);
				let wavG = sketch.map(sketch.sin(sketch.radians(250 + x * 0.8 + y)), -1, 1, 255, 0);
				let wavB = sketch.map(sketch.sin(sketch.radians(250 + x * 0.8 + y)), -1, 1, 0, 255);
